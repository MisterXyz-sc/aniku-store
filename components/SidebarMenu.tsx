...
import { Menu, X, Crown, Gem, Globe, Search, Home, History, RotateCcw } from 'lucide-react';
        ...
        <nav className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Link
              href="/lanjutkan"
              className={`flex items-center gap-3 rounded-xl px-2.5 py-2 transition ${
                pathname === '/lanjutkan' ? 'bg-ink-field' : 'hover:bg-ink-field/60'
              }`}
            >
              <div className="w-8 h-8 shrink-0 rounded-lg bg-brand/15 flex items-center justify-center">
                <RotateCcw size={15} strokeWidth={2.25} className="text-brand" />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-paper">Lanjutkan pembayaran</p>
                <p className="text-[11px] text-paper-muted truncate">Buat QRIS manual yang belum kelar</p>
              </div>
            </Link>
            <Link
              href="/riwayat"
